...
serve(async (req) => {
  ...
  try {
    const { resumeText, audience } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You generate elevator pitches. Return JSON: {pitch_30:"30 second pitch",pitch_60:"60 second pitch",pitch_90:"90 second pitch",tips:["delivery tips"]}` },
          { role: "user", content: `Generate elevator pitches from resume: ${resumeText?.substring(0, 3000)}. Target audience: ${audience || "General professional"}` },
        ],
        temperature: 0.7,
      }),
    });
