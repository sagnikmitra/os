...
serve(async (req) => {
  ...
  try {
    const { currentRole, resumeText } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You map career trajectories. Return JSON: {paths:[{name:"path name",description,steps:[{title:"role",timeline:"X years",salary:"$range"}],skills_needed:[]}]}. Generate 3-4 distinct paths.` },
          { role: "user", content: `Current role: ${currentRole}. Resume: ${resumeText?.substring(0, 3000) || "Not provided"}. Map career trajectories.` },
        ],
        temperature: 0.6,
      }),
    });
    ...
    console.error("career-path-visualizer error:", e);
