  ...
  try {
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You optimize LinkedIn profiles. Return JSON: {headlines:["3-5 headline options"],about:"optimized about section ~300 words",experience:[{title:"role at company",description:"optimized description"}],keywords:["relevant keywords"]}` },
          { role: "user", content: `Optimize LinkedIn profile from this resume: ${resumeText?.substring(0, 5000)}` },
        ],
        temperature: 0.7,
      }),
    });
