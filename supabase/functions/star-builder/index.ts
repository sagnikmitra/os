...
serve(async (req) => {
  ...
  try {
    const { achievement, targetRole } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You build STAR stories from resume achievements. Return JSON: {stories:[{question:"behavioral question this answers",situation,task,action,result,tips:"delivery tip",follow_up_questions:[]}]}. Generate 2-3 stories.` },
          { role: "user", content: `Achievement: "${achievement}"${targetRole ? `. Target role: ${targetRole}` : ""}. Build STAR stories.` },
        ],
        temperature: 0.7,
      }),
    });
    ...
    console.error("star-builder error:", e);
