...
serve(async (req) => {
  ...
  try {
    const { bulletPoint, targetAudience } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You build detailed project case studies from brief bullet points. Return JSON: {title,overview,challenge,approach,solution,results:[],lessons_learned:[],technologies:[]}` },
          { role: "user", content: `Build a detailed case study from: "${bulletPoint}". Target audience: ${targetAudience || "Technical hiring managers"}.` },
        ],
        temperature: 0.7,
      }),
    });
    ...
    console.error("case-study-builder error:", e);
