  ...
  try {
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You provide salary benchmark data. Return JSON: {percentile_25:"$X",median:"$X",percentile_90:"$X",total_comp:{base:"range",bonus:"range",equity:"range",benefits:"value"},factors:["factor1"],negotiation_tips:["tip1"]}` },
          { role: "user", content: `Salary benchmark for: ${role}, Location: ${location || "US average"}, Experience: ${experience}. Provide realistic market data.` },
        ],
        temperature: 0.3,
      }),
    });
    ...
    console.error("salary-benchmark error:", e);
