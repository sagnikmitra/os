  ...
  try {
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You research companies for job seekers. Return JSON: {overview:{description,industry,size,founded,recent_news:[]},culture:{summary,values:[],pros:[],cons:[]},interview:{process,common_questions:[],tips:[]},financials:{summary,salary_range},application_tips:[]}` },
          { role: "user", content: `Research ${companyName} for a job seeker. Provide comprehensive insights.` },
        ],
        temperature: 0.5,
      }),
    });
