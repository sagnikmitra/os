...
serve(async (req) => {
  ...
  try {
    const { resumeText, context } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You generate professional bios in multiple lengths. Return JSON: {twitter_bio:"~160 chars",linkedin_headline:"~120 chars",conference_bio:"~100 words",speaker_bio:"~150 words",professional_bio:"~200 words",casual_bio:"~80 words"}` },
          { role: "user", content: `Generate bios from resume: ${resumeText?.substring(0, 3000)}. Context: ${context || "General professional use"}.` },
        ],
        temperature: 0.7,
      }),
    });
    ...
    console.error("bio-generator error:", e);
