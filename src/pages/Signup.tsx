...
import { Eye, EyeOff, ArrowRight, Loader2, CheckCircle2, Shield, Hammer, Briefcase, Mic, TrendingUp, Mail } from "lucide-react";
...
export default function Signup() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.log("[Signup] Google OAuth result:", JSON.stringify(result));
      if (result?.error) {
        ...
        <div className="relative z-10 flex flex-col justify-between p-10 lg:p-14 w-full">
          ...
          <div className="max-w-md">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed mb-8">
              From resume analysis to interview prep, salary benchmarks to LinkedIn optimization — everything you need to land your next role.
            </p>
            <div className="grid grid-cols-2 gap-2">
              {HIGHLIGHTS.map((h) => (
                <div key={h.label} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border border-border/50 bg-card/50">
        ...
        <div className="flex-1 flex items-center justify-center px-6 pb-12">
          <div className="w-full max-w-sm">
            <div className="text-center mb-8">
              ...
              <h1 className="font-display text-3xl font-bold tracking-[-0.02em] mb-2">Create your account</h1>
              <p className="text-sm text-muted-foreground">Start building professional resumes in minutes.</p>
            </div>
            <Button variant="outline" className="w-full gap-3 h-12 mb-6 rounded-xl border-border/60 hover:border-primary/20 hover:bg-secondary transition-all" onClick={handleGoogleSignIn} disabled={googleLoading}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="name" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="John Doe" required className="h-11 rounded-xl" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Re-enter password" required className="h-11 rounded-xl" />
              ...
              <Button type="submit" className="w-full gap-2 h-11 rounded-xl shadow-md shadow-primary/15" disabled={loading || (!!confirmPassword && password !== confirmPassword)}>
